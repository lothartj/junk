...
def run(playwright: Playwright) -> None:
    ...
    # Attempt to click the first button with the special character
        ...
        page.frame_locator("iframe[title=\"Main Content\"]").get_by_placeholder("Search").fill(f"{identifier}")
        time3()
        ...
        page.frame_locator("iframe[title=\"Main Content\"]").get_by_role("button", name=f"No., sorted in Ascending order {identifier}").click()
